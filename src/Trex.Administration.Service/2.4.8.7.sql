...
(
   ...
   WHERE 
		...
		AND te.endtime <= DATEADD(S, -1, DATEADD(D, 1, @enddate)) AND
...
VALUES ('2.4.8.7', GETDATE(), 'RAB', 'Corrected FindAllTimeEntries- and FindBillableTimeEntiress WHERE criteria, spGenerateNewInvoiceDraft now set DueDate to 14 days after the InvoiceDate and spGetCustomersInvoiceView now finds draft better')
GO